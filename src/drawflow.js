export default class Drawflow {
  constructor(container, render = null, parent = null) {
    ...
    this.node_selected_startx = 0;
    this.node_selected_starty = 0;
  ...
  click(e) {
    ...
    if (this.editor_mode === "fixed") {
      ...
      if (e.button === 0) {
        if (e.target.classList[0] != "drawflow-resize") {
          this.resizing = false;
          this.contextmenuDel();
        } 
        else {
          ...
          this.node_selected_startx = parseInt(document.defaultView.getComputedStyle(this.node_selected).width, 10);
          this.node_selected_starty = parseInt(document.defaultView.getComputedStyle(this.node_selected).height, 10);

          this.mouse_startx = e.clientX;
          this.mouse_starty = e.clientY;
        }
      }
    ...
    switch (this.ele_selected.classList[0]) {
      ...
      // case "drawflow-resize":
      //   console.log("drawflow resize called");
      //   //return;
      //   break;
  ...
  position(e) {
    ...
    if (this.resizing)
    {
      var iWidth = (this.node_selected_startx + e.clientX - this.mouse_startx);
      this.node_selected.style.width = iWidth + 'px';

      //this.node_selected.style.height = (this.node_selected_starty + e.clientY - this.mouse_starty) + 'px';

      var inputs = this.node_selected.querySelectorAll("input");

      //console.log(inputs);

      inputs.forEach(element => {
        element.style.width = iWidth - 40 + 'px';
      });
  ...
  contextmenu(e) {
    ...
    if (this.precanvas.getElementsByClassName("drawflow-resize").length) {
      this.precanvas.getElementsByClassName("drawflow-resize")[0].remove();
    }
    if (this.node_selected || this.connection_selected) {
      ...
      resizebox.classList.add("drawflow-resize");
      resizebox.innerHTML = "";

      if (this.node_selected) {
        this.node_selected.appendChild(deletebox);
        this.node_selected.appendChild(resizebox);
      }
      ...
      if (
        ...
        this.precanvas.appendChild(resizebox);
      }
    }
  }
  contextmenuDel() {
